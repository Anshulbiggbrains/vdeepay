...
function genQuery(row, keys, value, defaultQuery) {
  ...
  if (keys && row) {
    for (let i = 0; i < keys.length; i++) {
      // console.log("find=>", row?.length > 0 && row.findAll(keys));
          ...
          onChange={(event, newValue) => {
            if (newValue) {
              ...
              if (apiResponse && apiResponse.length > 0) {
                ...
                // console.log("found else=>", newValue);
                setValue(newValue);
                if (cb1) cb1(newValue);
              }
            } else {
